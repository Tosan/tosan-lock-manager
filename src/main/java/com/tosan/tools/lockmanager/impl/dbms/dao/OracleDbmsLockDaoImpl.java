...
import static com.tosan.tools.lockmanager.impl.dbms.dao.OracleLockMode.SUB_SHARED_MODE;
...
public class OracleDbmsLockDaoImpl implements DbmsLockDao {
    ...
    public void requestLock(String lockNameType, String lockName, Integer timeout, boolean releaseOnCommit,
        ...
        switch (requestLockType) {
            case READ -> requestLock(
                    ...
                    SUB_SHARED_MODE, timeout, releaseOnCommit);
            case WRITE -> requestLock(
                    entityManager,
                    getLockHandle(entityManager, getSchemaName(), lockNameType, lockName),
                    OracleLockMode.EXCLUSIVE_MODE, timeout, releaseOnCommit);
    ...
    public void convertLock(String lockNameType, String lockName, Integer timeout,
        ...
        if (convertRequestLockType == null) {
            throw new LockManagerRunTimeException("request lock type is null.");
        }
        switch (convertRequestLockType) {
            case READ -> convertLock(
                    entityManager,
                    getLockHandle(entityManager, getSchemaName(), lockNameType, lockName),
                    SUB_SHARED_MODE, timeout);
            case WRITE -> convertLock(
                    entityManager,
                    getLockHandle(entityManager, getSchemaName(), lockNameType, lockName),
                    OracleLockMode.EXCLUSIVE_MODE, timeout);
            default -> throw new LockManagerRunTimeException("Invalid request lock type " + convertRequestLockType);
        }
    }
    ...
    private String getLockHandle(EntityManager entityManager, String schemaName, String lockNameType, String lockName) {
    ...
    private void requestLock(final EntityManager entityManager, final String lockHandle, final OracleLockMode lockMode,
    ...
    private void convertLock(final EntityManager entityManager, final String lockHandle, final OracleLockMode lockMode,
        ...
        try {
            callStatus = (Number) entityManager
                    ...
                    .setParameter("lockMode", lockMode.getValue())
    ...
    private void releaseLock(final EntityManager entityManager, final String lockHandle) {
